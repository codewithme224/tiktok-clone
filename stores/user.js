...
export const useUserStore = defineStore('user', {
    ...
    actions: {
      ...
      async updateUserImage(data) {
        return await $axios.post('http://localhost:8000/api/update-user-image', data)
      },

      async updateUser(name, bio) {
        return await $axios.patch('http://localhost:8000/api/update-user', {
          name: name,
          bio: bio
        })
      },
      ...
      async createPost($data) {
        console.log($data)
        return await $axios.post('http://localhost:8000/api/posts', $data)
      },

      async deletePost(post) {
        return await $axios.delete(`http://localhost:8000/api/posts/${post.id}`)
      },

      async addComment(post, comment) {
        let res = await $axios.post('http://localhost:8000/api/comments', {
          post_id: post.id,
          comment: comment
        })
      ...
      async updateComments(post) {
        let res = await $axios.get(`http://localhost:8000/api/profiles/${post.user.id}`)
      ...
      async deleteComment(post, commentId) {
        let res = await $axios.delete(`http://localhost:8000/api/comments/${commentId}`, {
          post_id: post.id
        })
      ...
      async likePost(post, isPostPage) {
        let res = await $axios.post('http://localhost:8000/api/likes', {
          post_id: post.id,
        })
      ...
      async unlikePost(post, isPostPage) {
        ...
        let res = await $axios.delete('http://localhost:8000/api/likes/' + deleteLike.id)
